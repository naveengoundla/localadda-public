...
export function StoreCard({ store, citySlug }: { store: Store; citySlug: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden flex"
      ...
      {/* Left — square image / emoji — clicking goes to store page */}
      <Link href={`/${citySlug}/${store.category.slug}/${store.slug}`} className="relative flex-shrink-0 w-28 h-28 sm:w-32 sm:h-32">
        {store.bannerUrl ? (
      ...
      {/* Right — info — clicking goes to store page */}
      <Link href={`/${citySlug}/${store.category.slug}/${store.slug}`}
        className="flex-1 px-3.5 py-3 flex flex-col justify-between min-w-0">
        <div>

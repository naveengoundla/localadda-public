        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-5 flex gap-5 overflow-x-auto scrollbar-hide">
          <Link href={`/${citySlug}${searchQuery ? `?q=${searchQuery}` : ''}`}
            ...
            <div className={`cat-icon ${!filterCategory ? 'active' : ''}`}
              style={{ background: !filterCategory ? 'linear-gradient(135deg,#f5a623,#e8401c)' : 'rgba(255,255,255,0.15)' }}>
              🏪
            </div>
            <span className="text-xs font-bold" style={{ color: !filterCategory ? '#fff' : 'rgba(255,255,255,0.5)' }}>All</span>
          </Link>
            ...
            return (
              <Link key={cat.slug} href={`/${citySlug}?category=${cat.slug}${searchQuery ? `&q=${searchQuery}` : ''}`}
                ...
                <div className={`cat-icon ${active ? 'active' : ''}`}
                  style={{ background: active ? (CAT_GRADIENT[cat.slug] || 'rgba(255,255,255,0.15)') : 'rgba(255,255,255,0.15)' }}>
                  {cat.emoji}
                </div>
                <span className="text-xs font-bold text-center" style={{ color: active ? '#fff' : 'rgba(255,255,255,0.5)', width: 60 }}>
                  {cat.name.split(' ')[0]}
                </span>
              </Link>
            );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        {(searchQuery || filterCategory) && (
          <p className="text-sm font-medium mb-5" style={{ color: '#888896' }}>
            ...
            {searchQuery && <span> for "<strong style={{ color: '#1a1a2e' }}>{searchQuery}</strong>"</span>}
            {filterCategory && <span> in <strong style={{ color: '#1a1a2e' }}>{uniqueCategories.find(c => c.slug === filterCategory)?.name}</strong></span>}
          </p>
        )}

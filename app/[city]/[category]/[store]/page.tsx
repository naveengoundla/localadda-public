...
export default async function StorePage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f4f5f7" }}>
      {/* Header */}
      <header style={{ background: "linear-gradient(135deg, #1a1a2e, #0f3460)" }} className="px-4 sm:px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          <Link href="/" className="text-white/50 hover:text-white text-sm hidden sm:block">LocalAdda</Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
          ...
          {/* ── Left column (2/3) — products, gallery ── */}
          <div className="lg:col-span-2 space-y-5">
            ...
            {allItems.length > 0 && (
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <h2 className="font-black text-gray-900 text-lg mb-4">📦 Products & Prices</h2>
                {/* 2-col product grid on desktop */}
            ...
            {store.galleryUrls && store.galleryUrls.length > 0 && (
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <h2 className="font-black text-gray-900 text-lg mb-4">📸 Photos</h2>
          ...
          {/* ── Right column (1/3) — contact, hours, CTA ── */}
          <div className="mt-5 lg:mt-0 space-y-5">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                {store.phone && (
                  <a href={`tel:${store.phone}`} className="flex items-center gap-3 py-2 hover:bg-gray-50 rounded-xl px-2 -mx-2">
                    <span className="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center text-lg flex-shrink-0">📞</span>
                    <div>
                ...
                {store.address && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-3 py-2 hover:bg-gray-50 rounded-xl px-2 -mx-2"
                    style={mapsUrl ? {} : { pointerEvents: 'none' }}
                  >
                    <span className="w-9 h-9 bg-blue-100 rounded-full flex items-center justify-center text-lg flex-shrink-0">📍</span>
